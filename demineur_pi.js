...
function revealAdjacentCells(row, col) {
  ...
  while (stack.length > 0) {
    ...
    if (!currentCell || currentCell.classList.contains("flagged")) continue; 
...
let currentRow = 0, currentCol = 0, press=true;
...
function utiliserDonneesJoystick(x,y,press){
  currentRow=x
  currentCol=y
  mode=press
  updateCursor(x,y)
}
function utiliserDonneesJoystick2(x, y) {
    // Décider du mouvement du curseur en fonction des valeurs x et y du joystick
    if (y < 0 && currentRow > 0) {  // Déplacement vers le haut
        currentRow--;
    } else if (y > 0 && currentRow < rows - 1) {  // Déplacement vers le bas
        currentRow++;
    }

    if (x < 0 && currentCol > 0) {  // Déplacement vers la gauche
        currentCol--;
    } else if (x > 0 && currentCol < cols - 1) {  // Déplacement vers la droite
        currentCol++;
    }
...
function boutonAppuye() {
  if (mode===true){
    ...
    // Ajouter ici la logique pour traiter l'appui sur le bouton (par exemple, révéler une case ou poser un drapeau)
}
...
socket.onmessage = function (event) {
    try {
        ...
        if (data.position) {
            ...
            utiliserDonneesJoystick(x, y,data.press);  // Utilise les données du joystick pour déplacer le curseur
        } else if (data.event === "button_pressed") {
            boutonAppuye();  // Si c'est un événement de bouton appuyé, appelle la fonction boutonAppuye
        }
    } catch (error) {
...
// Gestion des erreurs du WebSocket
...
// Connexion WebSocket pour récupérer les données du joystick
