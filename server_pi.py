...
async def get_mine(request:Request)->str:
    ...
    if difficulty=="facile":
        ...
        champ=plateau_jeu_possible_facil(taille,nb_mines,[0,0])
        plateau_jeu = str(champ)
        matrice_champ=champ
        ...
        case_joueur=(0,0)
        case_U= liste_voisins(case_joueur, taille)+[case_joueur]
        champ=init_plateau_mine(taille, nb_mines,case_U)
        plateau_jeu = str(champ) 
        matrice_champ=champ
...
class GroveButton(object):
    ...
    def __init__(self, pin):
        # High = pressed
        self.__btn = ButtonTypedGpio(pin,False)
...
def on_press(t):
    global bouton
    bouton="button_pressed"
    time.sleep(0.1)
    print('Button is pressed')
    bouton=False
        
async def bouton_appuye(websocket):
    Grove = GroveButton
    from grove.helper import SlotHelper
...
async def websocket_endpoint(websocket:WebSocket):
    ...
    #current_websocket = websocket  # Associer le websocket pour envoyer des données
    await asyncio.gather( lire_joystick(websocket),bouton_appuye(websocket)  )
